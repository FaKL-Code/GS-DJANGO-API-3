...
def activateEmail(request, user, to_email):
    ...
    if email.send():
        messages.success(
            request, f'A verification link was sent to your email <b>{ to_email }</b> address! Please remember to check your spam folder.')
    else:
        messages.error(
            request, f'Problem sending emailt to {to_email}, check if you enter your real email')
...
def custom_login(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if user is not None:
                login(request, user)
                messages.success(request, f'Logged as {user.username}!')
                return redirect('homepage')
